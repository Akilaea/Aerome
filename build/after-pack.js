...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Aerome icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Aerome';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Aerome executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Aerome icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Aerome resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Aerome',
    '--set-version-string', 'ProductName', 'Aerome',
    '--set-version-string', 'CompanyName', 'Aerome',
