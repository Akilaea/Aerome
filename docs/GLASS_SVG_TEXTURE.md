# Aerome SVG Glass Texture

> **说明**：本文件继承自上游 Mineradio 的 `GLASS_SVG_TEXTURE.md`，描述的玻璃质感基线在 Aerome 派生作品中保持不变。CSS filter ID `mineradio-control-glass-filter` 等仍保留旧名作为内部技术标识，未在用户可见层面暴露。
