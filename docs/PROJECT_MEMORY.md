# Mineradio Project Memory（上游历史，仅作参考）

> **⚠️ 上游历史文档**：本文件继承自上游 Mineradio，记录的是上游开发过程中用户的偏好与决策。所有提及的版本号、发布事件、UI 决策都是上游历史，**不适用于 Aerome 派生作品**。Aerome 自己的新决策请记录到 `CHANGELOG.md`。保留本文件仅为后续开发者理解上游偏好上下文。
