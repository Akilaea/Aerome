# Mineradio AI Handoff（历史文档）

> **⚠️ 历史文档说明**：本文件继承自上游 Mineradio 项目，描述的是上游 v1.1.0 发布前的工作流上下文。所有路径（如 `E:\桌面\播放器软件\Mineradio\resources\app`）和 GitHub 链接（`XxHuberrr/Mineradio`）都是上游的，**不适用于本 Aerome 派生作品**。本文件仅作历史参考保留，Aerome 自己的工作规则以 `AGENTS.md` 和 `CHANGELOG.md` 为准。
