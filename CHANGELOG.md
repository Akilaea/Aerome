...
主窗口关闭按钮改为**最小化到系统托盘**后台运行，符合音乐播放器的常驻预期。

### 托盘最小化

- 点窗口关闭按钮（或 Alt+F4）不再直接退出，而是隐藏到系统托盘继续后台播放；进程常驻，音乐不中断。
- 系统托盘图标：**单击**恢复主窗口；**右键菜单**提供「显示主窗口」/「退出 Aerome」。真正退出只走托盘「退出」。
- 新增设置项「关闭缩到托盘」（DIY 玩家模式 → 视觉控制台 → 高级参数 → 直播/后台），默认开启；关闭后恢复传统「点 X 直接退出」行为。
- 偏好持久化到本地，重启保留。
