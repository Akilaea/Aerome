...
> **v1.4.0 新增托盘最小化**：点窗口关闭按钮改为缩到系统托盘后台继续播放（进程常驻、音乐不中断）；托盘单击恢复窗口，右键菜单退出。可在「DIY 玩家模式 → 视觉控制台 → 高级参数 → 直播/后台」的「关闭缩到托盘」开关切换回传统直接退出行为。
> **v1.3.0 酷狗改为登录模式**：旧版免登录酷狗端点已整体失效，改为扫码登录 `www.kugou.com` 后走官网接口，登录账号有权限的 VIP 歌曲也能播。
> **v1.2.2 修复了 v1.2.0/v1.2.1 安装后打不开的问题**：之前数据目录硬编码在 E 盘，无 E 盘的机器启动会崩溃；现已改为落在安装根目录下的 `data\`。同时新增嵌套安装目录（`D:\Aerome\Aerome\...`）自动清理。老版本用户覆盖安装最新版即可。

## 相对上游的二创改动

- **B 站音源接入**：新增 `/api/bili/search`、`/api/bili/detail`、`/api/bili/audio`、`/api/bili/subtitle` 四个免登录端点；音频流走 DASH 自动挑最高码率；搜索 tab 增加「B站」平台选项。
- **酷狗音源接入（登录模式）**：搜索 `/api/kugou/search` 免登录（msearch + MD5 签名）；播放走官网 `wwwapi.kugou.com/play/songinfo`，需扫码登录 `www.kugou.com` 抓 cookie（登录账号有权限的 VIP 歌曲也能播）；歌词走 krcs/lyrics 两步式 LRC 下载；搜索 tab 增加「酷狗」平台选项；三方跨音源 fallback 链 netease ↔ qq ↔ kugou（VIP/受限歌曲自动匹配同名同歌手的其它平台版本）。
- **托盘最小化**：点窗口关闭按钮（或 Alt+F4）改为隐藏到系统托盘后台继续播放，进程常驻；托盘图标单击恢复主窗口，右键菜单提供「显示主窗口」/「退出 Aerome」；新增「关闭缩到托盘」设置项（默认开启）可切换回直接退出行为。
- **缓存目录统一**：所有运行时缓存（cookie、beatmap、updates、Electron userData）落在安装根目录下的 `data\`，不再写入 C 盘；开发版回退到项目下 `.aerome-data\`；可通过 `AEROME_DATA_DIR` 环境变量覆盖。
...
- QQ 音乐搜索、登录态与音源补充接入
- **新增**：B 站视频音频免登录播放
- **新增**：酷狗音乐扫码登录接入（播放走官网接口，账号有权限的 VIP 歌曲可播）
- **新增**：网易云 / QQ / 酷狗 三方跨音源 fallback，VIP 受限歌曲自动匹配同名版本
- **新增**：窗口关闭最小化到系统托盘，后台继续播放
...
正式分发以 Release 资产中的 `Aerome-1.4.0-Setup.exe` 为准。安装包会创建桌面快捷方式。
...
酷狗接入使用官网登录模式：搜索免登录；播放需扫码登录 `www.kugou.com` 后走官网 web 接口，仅播放登录账号有权限的曲目（VIP 歌曲需账号具备会员权限），不提供任何绕过付费/会员/风控的能力；受限曲目尝试自动跨音源匹配其它平台的免费版本。
