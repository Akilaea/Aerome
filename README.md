...
> **v1.2.2 修复了 v1.2.0/v1.2.1 安装后打不开的问题**：之前数据目录硬编码在 E 盘，无 E 盘的机器启动会崩溃；现已改为落在安装根目录下的 `data\`。同时新增嵌套安装目录（`D:\Aerome\Aerome\...`）自动清理。老版本用户覆盖安装 v1.2.2 即可。
...
正式分发以 Release 资产中的 `Aerome-1.2.2-Setup.exe` 为准。安装包会创建桌面快捷方式。
