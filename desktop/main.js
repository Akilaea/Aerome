...
const QQ_LOGIN_PARTITION = 'persist:aerome-qqmusic-login';
const QQ_LOGIN_URL = 'https://y.qq.com/n/ryqq/profile';
const KUGOU_LOGIN_PARTITION = 'persist:aerome-kugou-login';
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('aerome-global-hotkey', { action });
}

function unregisterAeromeGlobalHotkeys() {
...
function configureAeromeGlobalHotkeys(bindings = []) {
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.AEROME_NO_DESKTOP_SHORTCUT === '1' || process.env.AEROME_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.AEROME_CREATE_DESKTOP_SHORTCUT === '1' || process.env.AEROME_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'Aerome desktop music player',
...
async function openKugouLoginWindow(owner) {
  ...
  return new Promise((resolve) => {
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      if (/^https?:\/\/([^/]+\.)?kugou\.com/i.test(url) || /^https?:\/\/([^/]+\.)?kgmusic\.com/i.test(url)) {
...
public class AeromeMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([AeromeMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('aerome-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('aerome-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('aerome-desktop-lyrics-state', desktopLyricsState);
}
...
if (-not ("AeromeNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class AeromeNativeWin {
...
$progman = [AeromeNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[AeromeNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [AeromeNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [AeromeNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [AeromeNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[AeromeNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[AeromeNativeWin]::SetParent($target, $script:workerw) | Out-Null
[AeromeNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('aerome-wallpaper-state', wallpaperState);
}
...
ipcMain.handle('aerome-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'aerome-export.json').replace(/[\\/:*?"<>|]+/g, '-');
...
ipcMain.handle('aerome-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
...
ipcMain.handle('aerome-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('aerome-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('aerome-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('aerome-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('aerome-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('aerome-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('aerome-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('aerome-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('aerome-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('aerome-wallpaper-set-enabled', async (_event, enabled, payload) => {
  try {
...
ipcMain.handle('aerome-wallpaper-update', async (_event, payload) => {
  try {
...
async function createWindow() {
  ...
  process.env.AEROME_DATA_DIR = AEROME_DATA_DIR;
  process.env.COOKIE_FILE = path.join(AEROME_DATA_DIR, '.cookie');
  process.env.QQ_COOKIE_FILE = path.join(AEROME_DATA_DIR, '.qq-cookie');
  process.env.AEROME_UPDATE_DIR = getUpdateDownloadDir();
  try {
