// ====================================================================
//  Aerome Server — 沉浸式音乐播放器本地 API
//  派生自 Mineradio (GPL-3.0, (C) XxHuberrr)
//  - 网易云搜索 / 歌曲URL / 封面/音频代理
//  - 扫码登录 (login_qr_*) + cookie 持久化 (./.cookie)
//  - 试听检测 (freeTrialInfo) + 全 quality 探测
//  - B 站（bilibili）免登录音源接入
...
// ---------- CORS: 仅允许 loopback 来源 ----------
// 桌面应用 Electron 自身、本机浏览器调试都来自 127.0.0.1 / localhost。
// 拒绝外网/局域网/恶意页面通过浏览器跨域调用本机 API。
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.aerome && pkg.aerome.update) || {};
  const repoHint = process.env.AEROME_UPDATE_REPOSITORY
  ...
  const owner = process.env.AEROME_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.AEROME_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.AEROME_UPDATE_MANIFEST
      || process.env.AEROME_UPDATE_MANIFEST_URL
      || process.env.AEROME_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.AEROME_UPDATE_MIRRORS || process.env.AEROME_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `Aerome-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Aerome-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('Aerome v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `Aerome/${APP_VERSION}` },
    });
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Aerome-${latestVersion}-Setup.exe`;
  ...
  try {
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('Aerome v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Aerome-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Aerome-${version || APP_VERSION}.exe`;
}
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'aerome-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  try {
    ...
    if (host.includes('bilivideo.cn') || host.includes('biliapi.net') || host.includes('akamaized.net')) {
...
function generateBuvid3() {
  ...
  return segs.map(n => Array.from({ length: n }, () => hex[Math.floor(Math.random() * 16)]).join('')).join('-');
}
function generateBuvidFp() {
  // buvid_fp 是 32 位小写 hex
...
function loadBiliCookie() {
  try {
    if (fs.existsSync(BILI_COOKIE_FILE)) {
      const raw = fs.readFileSync(BILI_COOKIE_FILE, 'utf8').trim();
      if (/^[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}$/i.test(raw)) return raw;
    }
  } catch (e) {}
  const fresh = generateBuvid3();
  try { fs.writeFileSync(BILI_COOKIE_FILE, fresh); } catch (e) {}
  return fresh;
}
const BILI_BUVID3 = loadBiliCookie();
const BILI_BUVID_FP = generateBuvidFp();
const BILI_HEADERS = {
  'User-Agent': UA,
  'Referer': 'https://www.bilibili.com/',
  'Origin': 'https://www.bilibili.com',
  'Cookie': `buvid3=${BILI_BUVID3}; buvid_fp=${BILI_BUVID_FP}; buvid4=${generateBuvid3()}`,
};
...
function stripBiliHighlight(s) {
  return String(s || '').replace(/<[^>]+>/g, '').trim();
}
async function biliFetchJson(urlPath) {
  const u = urlPath.startsWith('http') ? urlPath : ('https://api.bilibili.com' + urlPath);
  const resp = await fetch(u, { headers: BILI_HEADERS });
  const text = await resp.text();
  try { return JSON.parse(text); } catch (e) {
    throw new Error('bili json parse failed: ' + text.slice(0, 120));
  }
}
async function handleBiliSearch(keywords, page, limit) {
  if (!keywords) return { total: 0, items: [] };
  const pg = Math.max(1, parseInt(page || '1', 10) || 1);
  const ps = Math.max(1, Math.min(50, parseInt(limit || '20', 10) || 20));
  const u = new URL('https://api.bilibili.com/x/web-interface/search/type');
  ...
  const data = await biliFetchJson(u.toString());
  if (data.code !== 0) throw new Error(data.message || ('bili code ' + data.code));
  const items = ((data.data && data.data.result) || []).map(v => ({
    bvid: v.bvid,
    aid: v.aid,
    title: stripBiliHighlight(v.title),
    author: v.author || '',
    duration: v.duration || '',
    play: v.play || 0,
    pic: normalizeBiliPic(v.pic),
    description: stripBiliHighlight(v.description),
  }));
  return {
    total: (data.data && data.data.numResults) || items.length,
    page: pg,
    items,
  };
}
async function handleBiliDetail(bvid) {
  if (!bvid) throw new Error('missing bvid');
  const data = await biliFetchJson('/x/web-interface/view?bvid=' + encodeURIComponent(bvid));
  if (data.code !== 0) throw new Error(data.message || ('bili code ' + data.code));
  const v = data.data || {};
  return {
    bvid: v.bvid,
...
async function handleBiliAudio(bvid, cid) {
  if (!bvid) throw new Error('missing bvid');
  let actualCid = cid;
  if (!actualCid) {
    const d = await handleBiliDetail(bvid);
    actualCid = d.cid;
  }
  const u = `https://api.bilibili.com/x/player/playurl?bvid=${encodeURIComponent(bvid)}&cid=${encodeURIComponent(actualCid)}&fnval=16&qn=64`;
  const data = await biliFetchJson(u);
  if (data.code !== 0) throw new Error(data.message || ('bili code ' + data.code));
  const dash = data.data && data.data.dash;
  if (!dash || !Array.isArray(dash.audio) || !dash.audio.length) {
    throw new Error('no dash audio stream');
  }
  // 按 bandwidth 降序，挑最高码率；过滤掉备份流（baseUrl 为空的）
  const audio = dash.audio
    .filter(a => a && a.baseUrl)
    .sort((a, b) => (b.bandwidth || 0) - (a.bandwidth || 0))[0];
  if (!audio) throw new Error('no playable audio stream');
  return {
    url: audio.baseUrl,
    audioId: audio.id,
    ...
    backupUrls: (audio.backupUrl || audio.backup_url || []).slice(0, 3),
    cid: actualCid,
    duration: (data.data && data.data.timelength) ? Math.round(data.data.timelength / 1000) : null,
  };
}
async function handleBiliSubtitle(bvid, cid) {
  if (!bvid) throw new Error('missing bvid');
  let actualCid = cid;
  if (!actualCid) {
    const d = await handleBiliDetail(bvid);
  ...
  const data = await biliFetchJson(`/x/player/v2?bvid=${encodeURIComponent(bvid)}&cid=${encodeURIComponent(actualCid)}`);
  if (data.code !== 0) return { lines: [], lan: '', lanDoc: '' };
  const subs = ((data.data || {}).subtitle || {}).subtitles || [];
  if (!subs.length) return { lines: [], lan: '', lanDoc: '' };
  // 优先中文字幕
  const pick = subs.find(s => /^zh/i.test(s.lan || '')) || subs[0];
  let subUrl = pick.subtitle_url || '';
  if (!subUrl) return { lines: [], lan: pick.lan, lanDoc: pick.lan_doc };
  if (subUrl.startsWith('//')) subUrl = 'https:' + subUrl;
  const subResp = await fetch(subUrl, { headers: BILI_HEADERS });
  const subData = await subResp.json();
  const body = Array.isArray(subData.body) ? subData.body : [];
  return {
    lan: pick.lan || '',
    lanDoc: pick.lan_doc || '',
    lines: body.map(l => ({
      from: typeof l.from === 'number' ? l.from : parseFloat(l.from || '0') || 0,
...
// ---------- kugou (login-mode, v1.3.0) ----------
// routes: /api/kugou/search (login-free) | /api/kugou/song/url (needs login, wwwapi play/songinfo) | /api/kugou/lyric (login-free)
// dfid/mid: device/session ids, generated at startup + persisted to .kugou-dfid
// login cookie persisted to .kugou-login; KuGoo is a plain composite field (userid=&token=&vip_type=)
const KUGOU_DID_FILE = process.env.KUGOU_DID_FILE || path.join(AEROME_DATA_DIR, '.kugou-dfid');
const KUGOU_LOGIN_FILE = process.env.KUGOU_LOGIN_FILE || path.join(AEROME_DATA_DIR, '.kugou-login');
const KUGOU_SECRET = 'NVPh5oo715z5DIWAeQlhMDsWXXQV4hwt';
const KUGOU_SEARCH_URL = 'https://msearch.kugou.com/api/v3/search/song';
const KUGOU_SONGINFO_URL = 'https://wwwapi.kugou.com/play/songinfo';
...
let kugouLoginCookie = '';
try { if (fs.existsSync(KUGOU_LOGIN_FILE)) kugouLoginCookie = fs.readFileSync(KUGOU_LOGIN_FILE, 'utf8').trim(); }
catch (e) { kugouLoginCookie = ''; }
function saveKugouLogin(c) {
  kugouLoginCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  try { fs.writeFileSync(KUGOU_LOGIN_FILE, kugouLoginCookie); } catch (e) {}
}
...
// parse the KuGoo composite cookie field: "userid=..&token=..&vip_type=.." -> { userid, token, vipType, ... }
function decodeKugooField(raw) {
  const out = {};
  try {
...
// decode kugou name values that use JS-style %uXXXX escapes (decodeURIComponent can't handle them)
function decodeKugouUnicodeName(raw) {
  let s = String(raw || '').trim();
  if (!s) return '';
  if (/%u[0-9a-fA-F]{4}/.test(s)) {
    try {
      s = s.replace(/%u([0-9a-fA-F]{4})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
      try { s = decodeURIComponent(encodeURIComponent(s)); } catch (_) {}
    } catch (_) {}
    return s;
  }
  try { return decodeURIComponent(s.replace(/\+/g, ' ')); } catch (_) { return s; }
}
// returns parsed kugou login info from persisted cookie; loggedIn requires KugooID + t
function parseKugouLoginInfo() {
  const obj = parseCookieString(kugouLoginCookie);
  const kugoo = decodeKugooField(obj.KuGoo || obj.kugoo || obj.KUGOO || '');
  ...
  const vipType = Number(obj.vip_type || kugoo.vip_type || kugoo.vipType || 0) || 0;
  const nickname = decodeKugouUnicodeName(obj.UserName || kugoo.NickName || kugoo.nickname || kugoo.nick || kugoo.uname || obj.nickname || '');
  const pic = String(kugoo.Pic || kugoo.pic || kugoo.avatar || obj.avatar || '').trim();
  return {
    userid,
    token,
    vipType,
    nickname,
    avatar: normalizeKugouPic(pic),
    loggedIn: !!(userid && token),
  };
}
async function getKugouLoginInfo() {
  const info = parseKugouLoginInfo();
  return {
    provider: 'kugou',
...
async function handleKugouSongUrl(hash, albumId) {
  ...
  const login = parseKugouLoginInfo();
  if (!login.loggedIn) {
    const r = playbackRestriction('kugou', 'login_required', '酷狗需要登录后才能获取播放地址', 'login', {
      url: '', playable: false, loggedIn: false, hash: h,
    });
    return { ...r, reason: r.category };
  }
  const params = {
    srcappid: 2919,
    clientver: 20000,
  ...
  const targetUrl = kugouSignedUrl(KUGOU_SONGINFO_URL, params);
  let data;
  try {
    const text = await requestText(targetUrl, { headers: { ...KUGOU_WEB_HEADERS, Cookie: kugouLoginCookie } });
    data = parseJSONText(text);
  } catch (e) {
    const r = playbackRestriction('kugou', 'url_unavailable', '酷狗接口请求失败', 'switch_source', {
      url: '', playable: false, loggedIn: true, hash: h, error: e.message,
    });
    return { ...r, reason: r.category };
  }
  const status = Number(data && (data.status || data.err_code));
  const inner = (data && data.data) || {};
  const playUrl = String(inner.play_url || data.play_url || '').replace(/\\\//g, '/').trim();
  const privilege = Number(inner.privilege || data.privilege || 0);
  const hasPrivilege = Number(inner.is_free_part === 1 ? 1 : 0);
  const lyricText = String(inner.lyrics || data.lyrics || '').replace(/\\\//g, '/').trim();
  const playable = !!playUrl && privilege === 0 && status === 1;
  return {
    provider: 'kugou',
    url: playUrl,
    playable,
    loggedIn: true,
    duration: Number(inner.timelength || data.timelength || inner.timeLength || 0),
    pic: normalizeKugouPic(inner.img || inner.album_img || data.imgUrl || ''),
    lyric: lyricText,
    lines: parseKugouLrc(lyricText),
    hash: h,
    songName: stripKugouTag(inner.song_name || inner.audio_name || data.songName || ''),
    payRequired: !playable && (privilege !== 0 || hasPrivilege === 1),
...
const server = http.createServer(async (req, res) => {
  ...
  // 统一在最前面应用 CORS 头；预检直接 204 返回，不走后续路由
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'aerome',
      productName: APP_PACKAGE.productName || 'Aerome',
      version: APP_VERSION,
      update: {
  ...
  // ---------- B 站（免登录）----------
  if (pn === '/api/bili/search') {
    try {
      const keywords = url.searchParams.get('keywords') || '';
      const page = url.searchParams.get('page') || '1';
      const limit = url.searchParams.get('limit') || '20';
      const data = await handleBiliSearch(keywords, page, limit);
      sendJSON(res, { provider: 'bili', ...data });
    } catch (err) {
      console.error('[BiliSearch]', err);
      sendJSON(res, { provider: 'bili', error: err.message, items: [], total: 0 }, 500);
    }
    return;
  }

  if (pn === '/api/bili/detail') {
    try {
      const bvid = url.searchParams.get('bvid') || '';
      const data = await handleBiliDetail(bvid);
      sendJSON(res, { provider: 'bili', ...data });
    } catch (err) {
      console.error('[BiliDetail]', err);
      sendJSON(res, { provider: 'bili', error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/bili/audio') {
    try {
      const bvid = url.searchParams.get('bvid') || '';
      const cid = url.searchParams.get('cid') || '';
      const data = await handleBiliAudio(bvid, cid);
      sendJSON(res, { provider: 'bili', ...data });
    } catch (err) {
      console.error('[BiliAudio]', err);
      sendJSON(res, { provider: 'bili', error: err.message, url: '' }, 500);
    }
    return;
  }

  if (pn === '/api/bili/subtitle') {
    try {
      const bvid = url.searchParams.get('bvid') || '';
      const cid = url.searchParams.get('cid') || '';
      const data = await handleBiliSubtitle(bvid, cid);
      sendJSON(res, { provider: 'bili', ...data });
    } catch (err) {
  ...
  if (pn === '/api/kugou/login/cookie') {
    try {
      ...
      const info = parseKugouLoginInfo();
      if (!info.loggedIn) {
        sendJSON(res, { provider: 'kugou', loggedIn: false, error: 'INVALID_KUGOU_COOKIE', message: '酷狗 cookie 缺少有效 KuGoo 登录信息' }, 400);
        return;
      }
      const status = await getKugouLoginInfo();
      sendJSON(res, { ...status, saved: true });
    } catch (err) {
  ...
  if (pn === '/api/cover') {
    try {
      ...
      const controller = new AbortController();
      const onClientClose = () => { try { controller.abort(); } catch (e) {} };
      req.on('close', onClientClose);
      let resp;
      try {
        resp = await fetch(coverUrl, { headers: { 'User-Agent': UA, 'Referer': 'https://music.163.com/' }, signal: controller.signal });
      } catch (fetchErr) {
        if (controller.signal.aborted) { try { res.end(); } catch (e) {} return; }
        throw fetchErr;
      }
      ...
      } catch (streamErr) { /* 客户端断开或上游中断：正常退出 */ }
      try { res.end(); } catch (e) {}
      try { req.off('close', onClientClose); } catch (e) {}
    } catch (err) { console.error('[Cover]', err); try { res.writeHead(500); res.end(); } catch (e) {} }
    return;
  }
  ...
  if (pn === '/api/audio') {
    try {
      ...
      // 客户端提前断开时取消上游下载，避免无意义带宽和内存占用
      const controller = new AbortController();
      const onClientClose = () => { try { controller.abort(); } catch (e) {} };
      req.on('close', onClientClose);
      let up;
      try {
        up = await fetch(audioUrl, { headers: hdr, signal: controller.signal });
      } catch (fetchErr) {
        if (controller.signal.aborted) { try { res.end(); } catch (e) {} return; }
        throw fetchErr;
      }
      ...
      try {
        ...
        // 客户端断开或上游中断：正常退出，不报 500
      }
      try { res.end(); } catch (e) {}
      try { req.off('close', onClientClose); } catch (e) {}
    } catch (err) { console.error('[Audio]', err); try { res.writeHead(500); res.end(); } catch (e) {} }
    return;
  }
  ...
  const filePath = pn === '/' ? '/index.html' : pn;
  // path.resolve 把 pn 当作相对 PUBLIC_ROOT 的子路径解析，再显式校验仍在 PUBLIC_ROOT 下
  const resolved = path.resolve(PUBLIC_ROOT, '.' + filePath);
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' Aerome v' + (APP_PACKAGE.version || '?') + '  →  http://localhost:' + PORT);
